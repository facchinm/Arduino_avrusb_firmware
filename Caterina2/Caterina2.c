...
static bool _VENDOR_WEBUSB_REQUEST = false;
	...
	while(true) {
		...
		while(true) {
			...
			if (_VENDOR_WEBUSB_REQUEST)
				Endpoint_SelectEndpoint(WEBUSB_RX_EPADDR);
			else
...
void EVENT_USB_Device_ConfigurationChanged(void)
{
	...
	Endpoint_ConfigureEndpoint(WEBUSB_TX_EPADDR, EP_TYPE_BULK, CDC_TX_EPSIZE, CDC_TX_BANK_SIZE);

	Endpoint_ConfigureEndpoint(WEBUSB_RX_EPADDR, EP_TYPE_BULK, CDC_RX_EPSIZE, CDC_RX_BANK_SIZE);
}
...
void EVENT_USB_Device_ControlRequest(void)
{
	...
	if (USB_ControlRequest.wIndex == 2 && _VENDOR_WEBUSB_REQUEST == false) {
			_VENDOR_WEBUSB_REQUEST = true;
	}
	if (USB_ControlRequest.wIndex == 0 && _VENDOR_WEBUSB_REQUEST == true) {
			_VENDOR_WEBUSB_REQUEST = false;
	}
...
static void WriteNextResponseByte(const uint8_t Response)
{
	...
	if (_VENDOR_WEBUSB_REQUEST)
		Endpoint_SelectEndpoint(WEBUSB_TX_EPADDR);
	else
